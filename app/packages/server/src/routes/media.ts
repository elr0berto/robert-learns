...
media.post('/uploadFile/:workspaceId', upload.single('file'), async (req, res : TypedResponse<MediaUploadFileResponseData>, next) => {
    try {
        ...
        if (!await checkPermissions({
            ...
            capability: Capability.CreateCard
        })) {
        ...
        console.error('/media/uploadFile caught ex', ex);
        next(ex);
        return;
    }
});

media.get('/:mediaId/*', async (req, res, next) => {
    try {
        const user = await getSignedInUser(req.session);
        ...
        console.error('/media/:mediaId/* caught ex', ex);
        next(ex);
        return;
    }
});
