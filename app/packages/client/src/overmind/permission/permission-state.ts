...
function _userCan(state: PermissionState, rootState: typeof config.state, capability: Capability): boolean {
    return userCan(rootState.signIn.userId === null, rootState.page.workspace?.allowGuests ?? false, state.role, capability, rootState.signIn.user?.emailVerified ?? false);
}
